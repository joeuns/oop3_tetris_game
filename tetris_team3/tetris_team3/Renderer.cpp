...
void Renderer::drawStory() {
    system("cls");
    setColor(WHITE);
    gotoXY(10, 2); std::cout << "당신은 이삿짐 센터의 직원입니다!";
    gotoXY(10, 5); std::cout << "상자를 알맞은 위치에 배치해 빈 공간이 없도록 꼼꼼히 정리해주세요.";
    gotoXY(10, 7); std::cout << "한 줄이 모두 채워질 경우 해당 줄의 짐은 트럭으로 옮겨집니다!";

    gotoXY(10, 10); std::cout << "센터에서는 원룸, 신혼집, 대가족까지 다양한 집의 이사를 돕습니다.";
    gotoXY(10, 12); std::cout << "자신만의 노하우를 쌓아올려 실력을 키운 후, 더 큰 집의 이사를 시도하세요!";

    gotoXY(10, 15); std::cout << "가끔은 2 player 모드로 동료 직원과 경쟁하며 일을 해보는 건 어떨까요?";
    gotoXY(10, 17); std::cout << "이삿짐 센터의 에이스에 도전해보세요!";

    gotoXY(10, 20); std::cout << "자, 이제 이사를 시작할 시간이네요.";
    gotoXY(10, 22); std::cout << "당신의 작업에 행운을 빕니다!";
